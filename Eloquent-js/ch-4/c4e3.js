...
function arrayToList(listObj) {
    arr.push(listObj["value"]);
    if (listObj["rest"] != null) {
        arrayToList(listObj["rest"]);
    } else {
        console.log(arr);
    }
}
...
console.log(arrayToList(list3));

//2nd part to add into list
function perpend(list, element) {
...
console.log(perpend(list3));


//function to get element from list
var counter = 1;
function getParticularElement(list, position) {
    if (position == counter) {
		console.log("element you are searching for:", list["value"]);
        return list["value"];
    } 
    
     if (list["rest"] != null) {
        counter++;
        getParticularElement(list["rest"], position);
    }else{
        console.log("Element not found...lol");
    }

}

console.log(getParticularElement(listttt, 2));